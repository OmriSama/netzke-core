class DynamicLoading < Netzke::Base
  ...
  def configure(c)
    super
    c.tbar = %i[load_component load_in_window load_with_feedback load_window_with_simple_component load_with_params]
    c.bbar = %i[non_existing_component inaccessible config_only load_self_reloading load_dynamic_child]
  end
end
