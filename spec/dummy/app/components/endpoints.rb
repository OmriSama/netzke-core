class Endpoints < Netzke::Base
  ...
  def configure(c)
    super
    c.bbar = %i[with_response no_response multiple_argument_response array_as_argument callback_and_scope
                return_value non_existing multiple_arguments]
    c.tbar = %i[hash_argument batched_call raise_exception return_error callback]
  end
  ...
  def invoke_endpoint(ep, *params, configs)
    ep = 'non_existing_child__endpoint' if ep == 'non_existing'
